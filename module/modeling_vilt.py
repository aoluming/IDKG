...
class ViltSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, prefix=None, output_attentions=False):
...
class ViltLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, prefix=None, output_attentions=False):
        self_attention_outputs = self.attention(
            hidden_states=self.layernorm_before(hidden_states),  # in ViLT, layernorm is applied before self-attention
            attention_mask=attention_mask,
            head_mask=head_mask,
            prefix=prefix,
            output_attentions=output_attentions,
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(hidden_states, attention_mask, layer_head_mask, prefix, output_attentions)
...
class ViltPooler(nn.Module):
    def __init__(self, config):
        ...
        self.pooled_position = 0

    def forward(self, hidden_states):
        # We "pool" the model by simply taking the hidden state corresponding
        # to the first token.
        pointer_token_tensor = hidden_states[:, self.pooled_position]
        pooled_output = self.dense(pointer_token_tensor)
